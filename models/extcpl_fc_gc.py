...
class DualST(nn.Module):
    ...
    def __init__(self,dconf):
        ...
        self.ext_c = nn.Sequential(
            MoKLayer(self.dconf.ext_dim,self.mconf.ext_cp_channel),
        ...
        self.ext_p = nn.Sequential(
            MoKLayer(self.dconf.ext_dim*(self.dconf.len_period+self.dconf.len_trend),self.mconf.ext_cp_channel),
            nn.Dropout(self.mconf.ext_dropout),
            nn.GELU(),
            MoKLayer(self.mconf.ext_cp_channel,self.mconf.channel_c * self.dconf.dim_h * self.dconf.dim_w),
            nn.GELU()
        ...
        self.FC = MoKLayer(in_features=self.mconf.transformer_dmodel * (self.dconf.len_close),
        ...
        self.ext_l = nn.Sequential(
            MoKLayer(self.dconf.ext_dim,self.mconf.extnn_l_channel),
            nn.Dropout(self.mconf.ext_dropout),
            nn.GELU(),
        ...
        self.group_conv = nn.Sequential(
            MoKLayer(self.mconf.transformer_dmodel,self.dconf.dim_flow * self.dconf.dim_h * self.dconf.dim_w),
            nn.Dropout(0.2),
            nn.GELU(),
        )
        # nn.Sequential(
