...
class MoKLayer(nn.Module):
    #MoKLayer's usage equals to nn.Linear
    ...
    def forward(self, x):
        ...
        # 将输入展平为二维，保留最后一个维度
        x = x.view(-1, x.size(-1))  # (B * ..., hidden)
        score = F.softmax(self.gate(x), dim=-1)  # (B * ..., E)
        expert_outputs = torch.stack([self.experts[i](x) for i in range(self.n_expert)], dim=-1)  # (B * ..., Lo, E)
        output = torch.einsum("BHE,BE->BH", expert_outputs, score)  # (B * ..., H)
        # 恢复原始形状，除了最后一个维度
        return output.view(*original_shape[:-1], -1).contiguous()
