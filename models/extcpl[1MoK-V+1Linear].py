...
class DualST(nn.Module):
    ...
    def __init__(self,dconf):
        ...
        self.ext_c = nn.Sequential(
            MoKLayer(self.dconf.ext_dim,self.mconf.ext_cp_channel,experts_type="V"),
            nn.Dropout(self.mconf.ext_dropout),
            nn.GELU(),
            nn.Linear(self.mconf.ext_cp_channel,self.mconf.channel_c * self.dconf.dim_h * self.dconf.dim_w),
            nn.GELU()
        )
            # nn.Sequential(
        ...
        self.ext_p = nn.Sequential(
            MoKLayer(self.dconf.ext_dim*(self.dconf.len_period+self.dconf.len_trend),self.mconf.ext_cp_channel,experts_type="V"),
            nn.Dropout(self.mconf.ext_dropout),
            nn.GELU(),
            nn.Linear(self.mconf.ext_cp_channel,self.mconf.channel_c * self.dconf.dim_h * self.dconf.dim_w),
            nn.GELU()
        ...
        self.ext_l = nn.Sequential(
            MoKLayer(self.dconf.ext_dim,self.mconf.extnn_l_channel,experts_type="V"),
            nn.Dropout(self.mconf.ext_dropout),
            nn.GELU(),
